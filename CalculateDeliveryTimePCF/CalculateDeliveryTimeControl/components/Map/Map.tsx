...
const Map: React.FC<IMap> = ({
    ...
    const startMap = (): void => {
        if (!map) {
            ...
            setHomeMarker(addHomeMarker(userLocation));

            const homeLocation = new google.maps.LatLng(40.748817, -73.985428);
            setHomeMarker(addHomeMarker(homeLocation));

            // const distanceBetweenPositions = google.maps.geometry.spherical.computeDistanceBetween(userLocation, homeLocation);
            // console.log("Avstand: " + distanceBetweenPositions);
        }
    }
    useEffect(startMap, [map]);

    const defaultMapStart = (): void => {
        const defaultAddress = new google.maps.LatLng(40.748817, -73.985428);
    ...
    const coordinateToAddress = async (coordinate: GoogleLatLng) => {
        ...
        await geocoder.geocode({ location: coordinate }, function (results: any, status) {
            if (status === 'OK') {
                setMarker({
    ...
    return (
        <div className="map-container">
            <div ref={ref} style={{ height: "600px", width: "600px" }} className="map-container__map"></div>
        </div>
    );
